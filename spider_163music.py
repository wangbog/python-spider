...
class Downloader():
    ...
    def get_songurls_playlist(self,playlist):
    ...
    def get_songurls_album(self,album):
        '''进入所选专辑页面，得出歌单里每首歌各自的ID 形式就是“song?id=64006"'''
        url=self.main_url+'album?id=%d'% album
        re= self.session.get(url)   #直接用session进入网页，懒得构造了
        sel=Selector(text=re.text)   #用scrapy的Selector，懒得用BS4了
        songurls=sel.xpath('//ul[@class="f-hide"]/li/a/@href').extract()
        return songurls   #所有歌曲组成的list
    ...
    def work(self,itemid,itemtype):
        if itemtype == 1:  #playlist
            songurls=self.get_songurls_playlist(itemid)  #输入歌单编号，得到歌单所有歌曲的url
            dir_path=r'/Users/tony626/Downloads'
            for songurl in songurls:
                self.download_song(songurl,dir_path)     #下载歌曲
        elif itemtype == 2:  #album
            songurls=self.get_songurls_album(itemid)     #输入专辑编号，得到歌单所有歌曲的url
            dir_path=r'/Users/tony626/Downloads'
            for songurl in songurls:
                self.download_song(songurl,dir_path)     #下载歌曲
        elif itemtype == 3:  #single song
            dir_path=r'/Users/tony626/Downloads'
            self.download_song("/song?id=" + str(itemid),dir_path)         #下载歌曲


if __name__ == '__main__':
    d=Downloader()
    #d.work(2214059025,1)  #playlist
    #d.work(75314803,2)  #album
    d.work(125399,3)  # single song
